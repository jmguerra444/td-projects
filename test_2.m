...
for imIn = 1:1 %size(images,3)
    I = images_test(:,:,imIn);
    [fa, da] = vl_sift(I);
    [matches, scores] = vl_ubcmatch(da, dAllTransposed);
end
%% hyperparams
iterations = 1000;
threshold = 50;
...
for i = 1:iterations
    ...
    for j = 1:size(matches,2)
        reproj_pt = K*[R_est, t_est']*[xAll(matches(2,j),:),1]';
        reproj_pt(1,:) = reproj_pt(1,:)./reproj_pt(3,:); %2d x point from proj
        reproj_pt(2,:) = reproj_pt(2,:)./reproj_pt(3,:); %2d y point from proj
        reproj_pt(3,:) = reproj_pt(3,:)./reproj_pt(3,:);
        reproj_pt(3,:) = [];
