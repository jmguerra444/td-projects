function  [best_rot, best_trans] = getinitial_Extrinsecs(matches,fa, xAll, cameraParams,iterations, threshold)

best_rot = zeros(3, 3); 
best_trans = zeros(3, 1);
max_inliers = 0;

wb = waitbar(0, 'doing RANSAC...');
for i = 1:iterations
    wb = waitbar(i/iterations,wb, 'doing RANSAC...');
    %samples n=4 random points
    rand_n_points = randi(size(matches, 2), 4, 1);
...
disp("Found " + max_inliers + "inliers")
close(wb)
end